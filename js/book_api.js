$(function () {
    let book_list = [
        ['아직 멀었다는 말', '파르티잔 극장', '엘리너 올리펀트는 완전 괜찮아', '사랑을 사랑하는 것', '아홉번째 파도', '머리부터 천천히', '저녁이 깊다'],
        ['이달의 이웃비', '밤의 고아', '두번의 자화상', '너무 한낮의 연애', '달의아이' , '수상한 중고상점', '어느 날, 내 죽음에 네가 들어왔다'],
                ...
                .done(function (msg) {
                    ...
                    let data = origin.filter((val) => {
                        if (i == 3 && j == 1) {
                            ...
                            return val.datetime.substring(0, 4) == '2016' && val.thumbnail !== '';
                        }
                        else if (i == 2 && j == 4) {
                            return val.authors[0] == '박성현' && val.title.length < 20 && val.thumbnail !== '';
                        }
                        else if (i == 2 && j == 6) {
